...
const HomePage: NextPage = () => {
  ...
  return (
    <>
      ...
      <h1 className="mt-5 text-center text-3xl font-extrabold text-gray-900 dark:text-slate-50 md:text-5xl lg:text-6xl">
...
const UnauthenticatedContent: NextPage = () => {
  ...
  const handleAuthenticateButtonClick = async () => {
    const response = await signIn("credentials", { privateKey });
  ...
  return (
    <>
      <label
        htmlFor="private-key"
        className="mb-2 block text-lg font-medium text-gray-900 dark:text-slate-50"
      >
        Cheia privată
      </label>
      <textarea
        id="private-key"
        rows={6}
        className="mb-3 block w-9/12 rounded-lg border border-gray-300 bg-slate-100 p-2.5 text-sm text-gray-900 focus:border-blue-500 focus:ring-blue-500 dark:border-gray-600 dark:bg-gray-700 dark:text-slate-50 dark:placeholder-gray-400 dark:focus:border-blue-500 dark:focus:ring-blue-500 lg:w-3/12"
        placeholder="Introdu cheia privată aici..."
        value={privateKey}
        onChange={handleTextAreaChange}
      ></textarea>
      <div className="mt-2 bg-gradient-to-br from-red-600 to-red-500 bg-clip-text text-2xl font-bold text-transparent">
        {error}
      </div>
      <button
        type="button"
