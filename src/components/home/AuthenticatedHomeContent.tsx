...
const AuthenticatedHomeContent: FC<Props> = ({ session }) => {
  ...
  const handleSendButtonClick = () => {
    ...
    if (file.size > maxFileSizeInBytes) {
      setFileError(`Fișierul selectat depășește limita de ${getFormattedFileSize(maxFileSizeInBytes)}.`);
      return;
    }
        ...
        <>
          {!!numberOfNewReceivedFiles && (
            <div
              ...
              className="z-10 mb-12 rounded-lg border border-blue-300 bg-blue-50 bg-opacity-75 p-4 text-blue-800"
              role="alert"
            >
              ...
              <div className="mb-1 mt-2 text-sm">
                Ai primit {newFilesString}. {numberOfNewReceivedFiles === 1 ? 'Îl' : 'Le'} poți găsi în{' '}
                <Link href="/fisiere/#primite">
                  <strong>tabelul de fișiere primite</strong>
                </Link>
                . Pentru a îndepărta eticheta de &quot;nou&quot; de pe un fișier, va trebui să îl descarci sau să îl ștergi.
              </div>
            </div>
          )}
          ...
          <label
            ...
            className="mt-3 block text-center text-sm font-medium text-gray-900"
          >
            Fișier
          </label>
          <label
            ...
            className={classNames(
              'mt-1 flex h-64 w-full cursor-pointer flex-col items-center justify-center rounded-lg border-2 border-dashed bg-opacity-50   hover:bg-gray-100 hover:bg-opacity-75',
              {
            ...
            <div className="flex flex-col items-center justify-center pb-6 pt-5 text-center">
              {!dragging && (
                <>
                  ...
                  <p className="text-xs text-gray-500">Orice extensie este acceptată.</p>
                  {file && (
